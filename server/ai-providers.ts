    ...
    switch (provider) {
        ...
        case "sglang": {
            ...
            const customFetch = Object.assign(
                ...
                {
                    preconnect: (fetch as any).preconnect,
                },
            )
        ...
        case "dashscope": {
            ...
            // Add a custom fetch wrapper to fix DashScope's non-compliant tool_calls format
            const customFetch = Object.assign(
                async (url: RequestInfo | URL, options?: RequestInit) => {
                    const response = await fetch(url, options)
                    if (!response.body) {
                    ...
                    // Create a transform stream to fix DashScope tool_calls format
                    let buffer = ""
                    const decoder = new TextDecoder()
                            ...
                            while (
                                ...
                                if (message.startsWith("data: ")) {
                                    ...
                                    try {
                                        ...
                                        // Fix DashScope's tool_calls format
                                        // DashScope returns: tool_calls: [{ function: { name, arguments }, ... }]
                                        // OpenAI expects: tool_calls: [{ type: "function", function: { name, arguments }, index: 0, id: "..." }]
                                        if (delta?.tool_calls) {
                                        ...
                                        // Re-serialize and forward the corrected data
                                        controller.enqueue(
                                            new TextEncoder().encode(
                                                `data: ${JSON.stringify(data)}\n\n`,
                                            ),
                                        ...
                                        // If parsing fails, forward the original message
                                        controller.enqueue(
                                            new TextEncoder().encode(
                                                message + "\n\n",
                                            ),
        ...
        case "moonshot": {
            const apiKey = overrides?.apiKey || process.env.MOONSHOT_API_KEY
            const baseURL =
                overrides?.baseUrl ||
            ...
            model = moonshotProvider.chat(modelId)
            break
        }

        case "zhipu": {
            const apiKey = overrides?.apiKey || process.env.ZHIPU_API_KEY
            const baseURL =
                overrides?.baseUrl ||
            ...
            model = zhipuProvider.chat(modelId)
            break
        }
        ...
        case "nvidia": {
            const apiKey = overrides?.apiKey || process.env.NVIDIA_API_KEY
            const baseURL =
                overrides?.baseUrl ||
            ...
            model = nvidiaProvider.chat(modelId)
            break
        }

        default:
            throw new Error(
                `Unknown AI provider: ${provider}. Supported providers: bedrock, openai, anthropic, google, azure, ollama, openrouter, deepseek, siliconflow, sglang, gateway, edgeone, doubao, modelscope, dashscope, moonshot, zhipu, minimax, nvidia`,
            )
    }
